...
public class UnstructuredStorageWriterUtil {
    ...
    public static void validateParameter(Configuration writerConfiguration) {
        ...
        if (!supportedWriteModes.contains(writeMode)) {
            throw DataXException
                    .asDataXException(
                            ...
                            String.format(
                                    "仅支持truncate, append, nonConflict3种模式, 不支持您配置的writeMode模式 : [%s]", writeMode));
        }
        ...
        if (StringUtils.isBlank(encoding)) {
            ...
            try {
                ...
                throw DataXException.asDataXException(UnstructuredStorageWriterErrorCode.ILLEGAL_VALUE, String.format("不支持您配置的编码格式:[%s]", encoding), e);
            }
        }
        ...
        if (!Constant.FILE_FORMAT_CSV.equals(fileFormat)
            ...
            throw DataXException.asDataXException(
                    UnstructuredStorageWriterErrorCode.ILLEGAL_VALUE, String
                            .format("您配置的fileFormat [%s]错误, 支持csv, text两种.", fileFormat));
        }
    }
    ...
    public static void writeToStream(RecordReceiver lineReceiver,
        ...
        try {
            ...
            throw DataXException.asDataXException(
                    UnstructuredStorageWriterErrorCode.RUNTIME_EXCEPTION, "运行时错误, 请联系我们", e);
        } catch (IOException e) {
            throw DataXException.asDataXException(
                    UnstructuredStorageWriterErrorCode.Write_FILE_IO_ERROR, String.format("流写入错误 : [%s]", context), e);
        } finally {
    ...
    private static void doWriteToStream(RecordReceiver lineReceiver,
        ...
        UnstructuredWriter unstructuredWriter = TextCsvWriterManager.produceUnstructuredWriter(fileFormat, fieldDelimiter, writer);
